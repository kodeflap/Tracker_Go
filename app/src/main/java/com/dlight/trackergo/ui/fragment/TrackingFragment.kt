...
class TrackingFragment : Fragment(R.layout.fragment_tracking) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            val cancelTrackingDialogFragment =
                parentFragmentManager.findFragmentByTag(CANCEL_TRACKING_DIALOG_TAG) as CancelTrackingDialogFragment?
    ...
    private fun subscribeToObserver() {
        ...
        TrackingService.timeRunInMills.observe(viewLifecycleOwner, Observer {
            curTimeMills = it
            val formattedTime = TrackingUtility.getFormattedStopWatchTime(curTimeMills, true)
    ...
    private fun endRunAndSaveToDB() {
        map?.snapshot { bmp ->
            ...
            val avgSpeed =
                round((distanceInMeters / 1000f) / (curTimeMills / 1000f / 60 / 60) * 10) / 10f
            val dateTimeStamp = Calendar.getInstance().timeInMillis
            val caloriesBurned = ((distanceInMeters / 1000f) * weight).toInt()
            val run =
                Run(bmp, dateTimeStamp, avgSpeed, distanceInMeters, curTimeMills, caloriesBurned)
